...
class ProgressiveProfilingEngine:
    ...
    @uses_db_session
    def check_for_prompts(self, session) -> Optional[dict]:
        ...
        self._generate_prompts(session)
    ...
    @uses_db_session
    def accept_prompt(self, session, prompt_id: int) -> None:
    ...
    @uses_db_session
    def decline_prompt(self, session, prompt_id: int) -> None:
    ...
    def _generate_prompts(self, session) -> None:
        ...
        try:
            ...
            self._check_new_domain_interest(session, profile)

            # Règle 2 : Préférence de style de réponse
            self._check_response_style_preference(session, profile, stats)

        except Exception as e:
            logger.debug("Erreur generate prompts : %s", e)

    def _check_new_domain_interest(self, session, profile: dict) -> None:
    ...
    def _check_response_style_preference(
        self, session, profile: dict, stats: dict,
    ) -> None:
    ...
    def _create_prompt(
        self, session, prompt_key: str, message: str, action_data: dict,
    ) -> None:
    ...
    @uses_db_session
    def _is_cooldown_ok(self, session) -> bool:
